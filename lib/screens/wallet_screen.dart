...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> _submitRequest() async {
    ...
    if (_amountController.text.isEmpty ||
        ...
        _sourcePhoneController.text.isEmpty ||
        _selectedPaymentMethod == null ||
        _uploadedImageUrl == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'الرجاء إكمال جميع البيانات المطلوبة وتحميل صورة إثبات الدفع',
          ),
          backgroundColor: Colors.red,
        ),
      );
    ...
    try {
      ...
      // تنسيق اسم المستخدم على انستا باي إذا كانت طريقة الدفع هي إنستا باي
              ...
              onPressed: () {
                Clipboard.setData(ClipboardData(text: details)).then((_) {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('تم نسخ $details إلى الحافظة'),
                        backgroundColor: Colors.green,
                      ),
                    );
                  }
