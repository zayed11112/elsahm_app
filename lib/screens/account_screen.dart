...
class _AccountScreenState extends State<AccountScreen> {
  ...
  final Logger _logger = Logger('AccountScreen');
  final ImagePicker _picker = ImagePicker();
  final String _imgbbApiKey =
      'e5ca1f47577dd78e2b024ada3ecb6dd9'; // Your ImgBB API Key
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    try {
      ...
      _logger.severe("Image Upload Error: $e");
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<String?> _uploadToImgBB(File imageFile) async {
    ...
    _logger.info("Uploading image to ImgBB...");
    final response = await request.send();

    if (response.statusCode == 200) {
      final responseBody = await response.stream.bytesToString();
      final decodedResponse = jsonDecode(responseBody);
      _logger.fine("ImgBB Response: $decodedResponse");
      if (decodedResponse['success'] == true &&
        ...
        _logger.severe(
          "ImgBB upload failed: ${decodedResponse['error']?['message'] ?? 'Unknown error'}",
        );
        return null;
      }
    } else {
      _logger.severe("ImgBB upload failed with status: ${response.statusCode}");
      _logger.severe("Response body: ${await response.stream.bytesToString()}");
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final authProvider = Provider.of<AuthProvider>(
      context,
    ); // listen: true to react to logout
    ...
    return StreamBuilder<UserProfile?>(
      ...
      builder: (context, snapshot) {
        ...
        final isLoading =
            snapshot.connectionState == ConnectionState.waiting || _isUploading;
        ...
        if (!snapshot.hasData || userProfile == null) {
          ...
          return Scaffold(
            appBar: _buildAppBar(context, theme, authProvider),
            body: const Center(child: Text('لم يتم العثور على الملف الشخصي.')),
          ); // Profile not found.
        }
  ...
  Widget _buildUserProfileSection(
    ...
    return Column(
      children: [
        ...
        Text(
          userProfile.name.isNotEmpty ? userProfile.name : 'اسم المستخدم',
        ...
        OutlinedButton.icon(
          ...
          onPressed: () {
            // Navigate to EditProfileScreen with userProfile data
            // - Pass userProfile object to EditProfileScreen constructor
            // - Handle navigation result for profile updates
            _logger.info('Edit profile tapped');
            Navigator.push(
              context,
              MaterialPageRoute(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildInfoRow(
                  ...
                  'الكلية:',
                  userProfile.faculty.isNotEmpty ? userProfile.faculty : '-',
                  theme,
                ), // Display only Arabic faculty
                _buildInfoRow(
                  Icons.location_city_outlined,
                  'الفرع:',
                  userProfile.branch.isNotEmpty ? userProfile.branch : '-',
                  theme,
                ), // Display Branch
                _buildInfoRow(
                  Icons.calendar_today_outlined,
                  'الدفعة:',
                  userProfile.batch.isNotEmpty ? userProfile.batch : '-',
                  theme,
                ),
                _buildInfoRow(
                  Icons.badge_outlined,
                  'الرقم الجامعي:',
                  userProfile.studentId.isNotEmpty
    ...
    final housingGroups = [
      {
        ...
        'link': 'YOUR_WHATSAPP_HOUSING_LINK_1',
      },
      {
        'name': 'جروب تسكين 2',
        'icon': Icons.home_work_outlined,
        'link': 'YOUR_WHATSAPP_HOUSING_LINK_2',
      },
    ];
    final facultyGroups = [
      {
        'name': 'جروب حاسبات ومعلومات',
        'icon': Icons.computer,
        'link': 'YOUR_WHATSAPP_CS_LINK',
      },
      {
        'name': 'جروب هندسة',
        'icon': Icons.engineering,
        'link': 'YOUR_WHATSAPP_ENG_LINK',
      },
      {
        'name': 'جروب صيدلة',
        'icon': Icons.local_pharmacy_outlined,
        'link': 'YOUR_WHATSAPP_PHARM_LINK',
      },
      {
        'name': 'جروب أسنان',
        'icon': Icons.local_hospital_outlined,
        'link': 'YOUR_WHATSAPP_DENT_LINK',
      },
      {
        'name': 'جروب استفسار وتقديم',
        'icon': Icons.help_outline,
        'link': 'YOUR_WHATSAPP_INQUIRY_LINK',
      },
    ];
      ...
      children: [
        ...
        Center(
          child: CircleAvatar(
            ...
            child: Icon(Icons.groups, size: 40, color: Colors.grey[500]),
          ),
        ),
        ...
        ListView.separated(
          ...
          itemBuilder:
              (context, index) => _buildGroupLinkItem(
                context,
                theme,
                housingGroups[index]['icon']! as IconData,
                housingGroups[index]['name']! as String,
                Icons.chat,
                () {
                  _logger.info(
                    'Tapped WhatsApp Housing Group: ${housingGroups[index]['name']}',
                  );
                  // Implement URL launcher for WhatsApp housing groups
                  // - Use url_launcher package to open: housingGroups[index]['link']! as String
                  // - Add error handling for failed launches
                },
              ),
        ...
        Center(
          child: CircleAvatar(
            radius: 40,
            backgroundColor: Colors.grey[800],
            child: Icon(Icons.school, size: 40, color: Colors.grey[500]),
          ),
        ),
        const SizedBox(height: 12),
        ListView.separated(
          ...
          itemBuilder:
              (context, index) => _buildGroupLinkItem(
                context,
                theme,
                facultyGroups[index]['icon']! as IconData,
                facultyGroups[index]['name']! as String,
                Icons.chat,
                () {
                  _logger.info(
                    'Tapped WhatsApp Faculty Group: ${facultyGroups[index]['name']}',
                  );
                  // Implement URL launcher for WhatsApp faculty groups
                  // - Use url_launcher package to open: facultyGroups[index]['link']! as String
                  // - Add error handling for failed launches
                },
              ),
          separatorBuilder: (context, index) => const SizedBox(height: 8),
  ...
  Widget _buildTelegramGroupsSection(BuildContext context, ThemeData theme) {
    ...
    final telegramGroups = [
      {
        'name': 'سكن استديو (ولاد وبنات)',
        'icon': Icons.home_outlined,
        'link': 'YOUR_TELEGRAM_STUDIO_LINK',
      },
      {
        'name': 'سكن أولاد (أوضة/سرير)',
        'icon': Icons.boy,
        'link': 'YOUR_TELEGRAM_BOYS_LINK',
      },
      {
        'name': 'سكن بنات (أوضة/سرير)',
        'icon': Icons.girl,
        'link': 'YOUR_TELEGRAM_GIRLS_LINK',
      },
      {
        'name': 'شقق غرفتين',
        'icon': Icons.apartment,
        'link': 'YOUR_TELEGRAM_APT2_LINK',
      },
      {
        'name': 'شقق 3 غرف',
        'icon': Icons.holiday_village,
        'link': 'YOUR_TELEGRAM_APT3_LINK',
      },
      {
        'name': 'شقق 4 غرف',
        'icon': Icons.maps_home_work_outlined,
        'link': 'YOUR_TELEGRAM_APT4_LINK',
      },
      {
        'name': 'قرية سما العريش',
        'icon': Icons.location_on_outlined,
        'link': 'YOUR_TELEGRAM_SAMA_LINK',
      },
    ];
        ...
        ListView.separated(
          ...
          itemBuilder:
              (context, index) => _buildGroupLinkItem(
                context,
                theme,
                telegramGroups[index]['icon']! as IconData,
                telegramGroups[index]['name']! as String,
                Icons.send,
                () {
                  _logger.info(
                    'Tapped Telegram Group: ${telegramGroups[index]['name']}',
                  );
                  //  Implement URL launcher for Telegram groups
                  // - Use url_launcher package to open: telegramGroups[index]['link']! as String
                  // - Add error handling for failed launches
                },
              ),
          separatorBuilder: (context, index) => const SizedBox(height: 8),
                ...
                isSelectable
                    ? SelectableText(
                      ...
                      onTap:
                          isUrl
                              ? () {
                                // Implement URL launcher for contact links
                                // - Use url_launcher package to open URLs
                                // - Add error handling for failed launches
                                _logger.info('Tapped URL: $value');
                              }
                              : null,
                    )
                    : Text(
