...
class CheckoutService {
  ...
  Future<void> addCheckoutRequest(Map<String, dynamic> checkoutData) async {
    try {
      _logger.info(
        'إضافة طلب تسجيل خروج جديد للعقار: ${checkoutData['property_name']}',
      );
      _logger.info('بيانات الطلب: ${checkoutData.toString()}');

      // طباعة معلومات الاتصال للتشخيص
      _logger.info(
        'حالة الاتصال بـ Supabase: ${_supabase.auth.currentSession != null ? 'متصل' : 'غير متصل'}',
      );
      ...
      final sanitizedData = {
        ...
        'commission':
            checkoutData['commission'] is num
                ? checkoutData['commission']
                : double.parse('${checkoutData['commission']}'),
        'deposit':
            checkoutData['deposit'] is num
                ? checkoutData['deposit']
                : double.parse('${checkoutData['deposit']}'),
        'property_price':
            checkoutData['property_price'] is num
                ? checkoutData['property_price']
                : double.parse('${checkoutData['property_price']}'),
      };
      ...
      try {
        ...
        _logger.warning(
          'فشل إضافة البيانات باستخدام الطريقة الأولى: $innerError',
        );
        ...
        _logger.info(
          'تم إضافة طلب تسجيل الخروج بنجاح باستخدام upsert، استجابة: $response',
        );
      }
    } catch (e, stackTrace) {
  ...
  Future<List<Map<String, dynamic>>> getUserCheckoutRequests(
    String userId,
  ) async {
    try {
  ...
  Future<List<Map<String, dynamic>>> getPropertyCheckoutRequests(
    String propertyId,
  ) async {
    try {
  ...
  Future<void> updateCheckoutRequestStatus(
    String requestId,
    String status,
  ) async {
    try {
  ...
  Future<void> updateCheckoutFinancials(
    String requestId, {
    double? commission,
    double? deposit,
  }) async {
  ...
  Future<void> deleteCheckoutRequest(String requestId) async {
    try {
      ...
      await _supabase.from('checkout_requests').delete().eq('id', requestId);
    ...
    try {
      ...
      final response =
          await _supabase
              .from('properties')
              .select(
                '*, id, name, description, type, price, commission, deposit',
              )
              .eq('id', propertyId)
              .maybeSingle();
